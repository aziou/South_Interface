...
namespace SoftType_G_ZD
{
    public class csFunction : Mis_Interface_Driver.MisDriver
    {
        ...
        public static string strMinIb = "", strMaxIb = "";
        ...
        private static List<string> Get_VT_SB_JKDNBJDJL(string str_PkID, out List<string> Col_Seal)
        {
            ...
            try
            {
                ...
                DataCore.Global.GB_Base.MeterCheckDate = OldRead["datJdrq"].ToString().Trim();
                DataCore.Global.GB_Base.MeterXBDM = OldRead["intClfs"].ToString().Trim();
                ...
                #region 3000F 平均值
                string StrAvr = Get_METER_COMMUNICATION("102").ToString().Trim();
                string[] arrRjs;
                char[] csplit = { ',' };
                arrRjs = StrAvr.Split(csplit);
                #endregion
                if (arrRjs.Length >= 6)
                {
                    strOracleSQL_Value = strOracleSQL_Value + "','" + arrRjs[5];
                }
                else
                {
        ...
        private static List<string> Get_VT_SB_JKDNBJDWC(string str_PkID)
        {
            ...
            try
            {
                ...
                while (red.Read() == true)
                {
                    ...
                    strOracleSQL_Value = "";
                    strOracleSQL_Name = "";
                    if (!OutRunSampleInfo(ref proCol, red["chrProjectNo"].ToString().Trim()))
                    {
                        continue;
                    }
        ...
        private static List<string> Get_VT_SB_JKRJSWC(string PKId)
        {
            ...
            try
            {
                ...
                if (strValue.IndexOf(",") > 0)  //有误差值
                {
                    ...
                    catch(Exception exRjs)
                    {
                        strOracleSQL_Name = strOracleSQL_Name + "PJZ,";   //平均值
                        if (Get_METER_COMMUNICATION("102").ToString().Trim().Split(csplit).Length == 1)
                        {
