    ...
    public class csFunction : Mis_Interface_Driver.MisDriver
    {
        ...
        private static List<string> Get_VT_SB_JKDNBJDJL(string PK_ID,out List<string> Col_For_Seal)
        {
           ...
           // Runtime = watch.ElapsedMilliseconds.ToString() + "ms";
            ...
            try
            {
                ...
                {
                    if (OldRead["AVR_TOTAL_CONCLUSION"].ToString().Trim() == "不合格") continue;
        ...
        private static List<string> Get_VT_SB_JKDNBJDWC(string PK_ID)
        {
            ...
            try
            {
                ...
                //    strOracleSQL_Value = "";
                //    strOracleSQL_Name = "";
                //    if (red["AVR_ERROR_MORE"].ToString().Trim() == "") continue;
                //    if (!OutRunSampleInfo(ref proCol, red["AVR_PROJECT_NO"].ToString().Trim()))
                //    {
                //        continue;
                //    }
                ...
                //    strOracleSQL_Value = strOracleSQL_Value + "','" + strValue;
                //    strOracleSQL_Name = strOracleSQL_Name + "FZDLDM,";   //负载电流代码
                //    if (red["AVR_ITEM_NAME"].ToString().Trim().Contains("+20%") || red["AVR_ITEM_NAME"].ToString().Trim().Contains("120%"))
                //    {
                //        strValue = "14";//1.2Un
                //    }
                //    else
                ...
                //    strOracleSQL_Name = strOracleSQL_Name + "JLDM,";   //结论代码
                //    strValue = red["AVR_ERR_CONCLUSION"].ToString().Trim();
                //    strOracleSQL_Value = strOracleSQL_Value + "','" + ResultsCode(strValue);
        ...
        {
            string strResults = "";
            string strSQL = " SELECT AVR_RESULT_VALUE FROM METER_RESULTS where AVR_RESULT_ID='" + RESULT_ID + "' and FK_LNG_METER_ID='" + DataCore.Global.GB_Base.MeterId + "'";
        ...
        {
            string strResults = "";
            string strSQL = " SELECT AVR_ERROR_CONCLUSION FROM METER_ERROR where  FK_LNG_METER_ID='" + DataCore.Global.GB_Base.MeterId + "'";
        ...
        public ObservableCollection<Clou_Report.Model.MemberForZJ> SetMemberForZj(string CheckTime, List<string> lis_Id)
        {
            ...
            try
            {
                ...
                using (OleDbConnection conn = new OleDbConnection(DataCore.Global.GB_Base.AccessLink))
                {
                    ...
                    foreach (string Temp_id in lis_Id)
                    {
                        string sql = string.Format("Select * from METER_INFO WHERE DTM_TEST_DATE" + DataCore.Global.GB_Base.CheckTimeCondition + "#{0}# and PK_LNG_METER_ID='{1}'", CheckTime, Temp_id);
                        OleDbCommand cmd = new OleDbCommand(sql, conn);
                        OleDbDataReader Myreader = null;
                        Myreader = cmd.ExecuteReader();
                        ...
                        if (temp.Count >= 0)
                        {
                            temp[temp.Count - 1].StrElectric = (Get_METER_COMMUNICATION("01701", temp[ItemsCount - 1].StrMeterID).Split(csplit).Length > 0) ? Get_METER_COMMUNICATION("01701", temp[ItemsCount - 1].StrMeterID).Split(csplit)[0] : "";
